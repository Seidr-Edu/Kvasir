...
tp_select_portable_test_scope_with_build_env() {
  local suite="$1"
  local repo="$2"
  local log_file="$3"
  local -a candidates=()
  ...
  if [[ ${#candidates[@]} -eq 0 ]]; then
    tp_select_portable_test_scope "$repo"
    return $?
  fi
  ...
  for jdk in "${candidates[@]}"; do
    ...
    tp_log "[$suite] portable-scope probe with JDK $jdk"
    tp_select_portable_test_scope "$repo"
    rc=$?
    if [[ "$rc" -eq 0 ]]; then
      tp_build_env_suite_set "$suite" "SELECTED_JDK" "$jdk"
    ...
    if [[ "$rc" -eq 2 ]]; then
      if tp_build_env_should_retry_failure "${TP_TEST_SCOPE_FAILURE_CLASS:-unknown}" "${TP_TEST_SCOPE_FAILURE_LOG:-$log_file}" "$attempt_no" "${#candidates[@]}"; then
        tp_warn "[$suite] retrying after toolchain-sensitive portable-scope probe failure on JDK $jdk"
        continue
      fi
      return 2
    fi
